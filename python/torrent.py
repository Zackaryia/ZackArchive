# https://alexpdev.github.io/torrentfile/
# https://github.com/alexpdev/torrentfile?tab=readme-ov-file
import os
...
def create_torrent(config: util.Config, file, outfile):
    piece_size = config.torrent_peice_length(os.path.getsize(file))
    Path(os.path.dirname(outfile)).mkdir(parents=True, exist_ok=True)

    return Torrent(file, piece_size, outfile)

def add_torrent(config, torrent_file_path, storage_folder):
    with qbittorrentapi.Client(
        ...
        qbt_api.torrents_add(
            torrent_files = torrent_file_path,
            save_path = storage_folder,
        )
...
# region torrent utility, found at http://bittorrent.org/beps/bep_0052.html #
# Downloadable at url http://bittorrent.org/beps/bep_0052_torrent_creator.py
...
class Torrent:
    def __init__(self, path, piece_length, save_path=None):
        ...
        if save_path != None:
            self.save_torrent_file(save_path)
    ...
    def metafile(self, tracker, hybrid=False):
        """
    ...
    def save_torrent_file(self, outfile, tracker="None"):
        metafile = self.metafile(tracker)
        with open(outfile, 'wb') as f:
            f.write(encode(metafile))



    def info_hash_v2(self):
        return sha256(encode(self.info)).digest()
